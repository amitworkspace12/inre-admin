...
const BESCOMTransfer = ({page, setPage,  subPage, setSubPage}) => {
  ...
  const Clientcolumns = [
    {
        ...
        render: (text, record) => (
          <span className="custom-column-style font-[600] text-[#4B465C] opacity-90 text-[15px]">{text}</span>
        ),
      },
    {
...

    {
      ...
      render: (text, record) => (
        ...
        <img src={pencil} className='w-[22px] h-[22px] cursor-pointer' onClick={()=> navigate(`/servicemanagement/post-registration/bescom-transfer/${record.key}`)}/>
        <img src={eye} className='w-[22px] h-[22px] cursor-pointer' onClick={()=> navigate(`/servicemanagement/post-registration/bescom-transfer/${record.key}`)}/>
        </div>
      ),
    },
  ]
  ...
  return (
    ...
    <div className="p-3 w-[80%] grid gap-5 box-border pb-20 bg-[#F7F6F9]">
      ...
      <div className={`flex gap-8 bg-white p-2 h-[38px] rounded-t-[6px] font-publicsans text-[15px] font-[400] border-b-2 w-fit`}>
        ...
        <p className='pb-7 text-[#002550] cursor-pointer border-b-2 border-black'>BESCOM Transfer</p>
